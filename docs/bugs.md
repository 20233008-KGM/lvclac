...
## 버그 4 (Critical) — ~~입력 패널 `비우기`가 활성 숫자세트와 연결 기록을 삭제함~~ ✅ 수정 완료

**증상:** 입력 패널의 `비우기` 버튼은 "모든 입력값을 비웁니다"라고 안내하지만, 저장이 켜진 활성
숫자세트에서는 입력값만 초기화하지 않고 숫자세트 행 자체를 삭제한다.

**원인 경로:**
1. `ClearAllInputsButton`이 `resetInputs()`를 호출해 입력 상태를 `defaultInputs`로 바꾼다.
2. 자동 저장이 이를 의미 있는 입력값이 없는 상태로 판단한다.
3. `persistInputs()`의 빈 입력 분기가 로컬에서는 `clearDraft()`, 클라우드에서는
   `deleteNumberSet()`을 호출한다.
4. 클라우드의 `number_sets` 삭제는
   `20260722000000_number_set_cascade_delete.sql`의 `ON DELETE CASCADE` 때문에 연결된
   `order_history`와 `account_snapshots`까지 연쇄 삭제한다.

**영향:**
- 로컬 숫자세트: 활성 세트의 ID·이름·프리셋이 사라지고 다음 세트로 전환된다.
- 클라우드 숫자세트: 세트의 이름·입력·프리셋·메모·자동 스냅샷/롤오버 설정이 사라진다.
- 클라우드 연결 기록: 해당 세트의 주문 이력·계좌 스냅샷·기록 메모가 함께 삭제될 수 있다.
- 마이페이지의 명시적 세트 삭제는 삭제 전 기록 개수를 경고하지만, 입력 패널 `비우기`는 이 확인 절차를
  거치지 않는다.

**확인 방법:** 실제 사용자 데이터를 삭제하는 재현은 하지 않았다. 버튼 → 컨텍스트 상태 초기화 →
자동 저장 → 로컬/클라우드 삭제 함수 → FK 연쇄 삭제까지 코드와 마이그레이션 계약을 정적으로 추적했다.

**수정 방향:** 숫자세트의 존재 여부와 의미 있는 입력값의 존재 여부를 분리한다. `비우기`는 세트 행과
메타데이터·연결 기록을 유지한 채 입력 필드만 초기화하고, 세트 삭제는 마이페이지의 명시적 삭제 흐름에서만
수행해야 한다. 빈 입력 세트의 저장·복원·클라우드 존재 판정도 함께 조정해야 하므로 단순히 삭제 호출 하나만
제거해서는 안 된다.

**✅ 수정 완료 (2026-07-28):**
- `persistInputs()`의 빈 입력 삭제 분기를 제거했다. 빈 입력도 정상 저장값으로 취급해 로컬은 활성 세트를
  upsert하고, 클라우드는 기존 `number_sets` 행의 `inputs`·`preset_id`만 갱신한다.
- 빈 입력 클라우드 세트도 실제 존재하는 저장 세트로 판정해 활성화·복원·저장 위치 표시·로컬 복사가
  정상 동작하도록 조정했다.
- `비우기` 툴팁·확인창·로컬/클라우드 저장 안내의 한영 문구를 실제 동작에 맞게 수정했다.
- 로컬 세트 ID·이름·프리셋·다른 세트 보존, 클라우드 저장 경로의 비삭제 계약, 빈 입력 세트 존재 판정을
  회귀 테스트로 고정했다.
- 검증: 집중 59/59, 전체 Vitest 105 files / 779 tests, TypeScript 포함 production build 통과.
  실제 사용자 데이터로 파괴적 삭제 재현은 하지 않았다.
