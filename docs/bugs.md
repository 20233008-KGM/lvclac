...
**신규 발견 버그: 2건 (High 1, Medium 1) — 둘 다 수정 완료.** + 참고사항 2건(마이그레이션 이력 불일치는 해소, 나머지는 유지).
...
- 참고: 이메일 열거 방지 관점에서 "계정이 존재한다"는 사실 자체를 노출하는 트레이드오프가 있음 — 대표비서
  승인 하에 정확한 안내를 우선하는 쪽으로 결정.
