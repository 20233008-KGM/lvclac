...
## 버그 4 (High) — ~~공개판 입력 `비우기`가 로컬 저장값과 저장 상태를 삭제함~~ ✅ 수정 완료

**증상:** 공개 계산기의 입력 패널에서 `비우기`를 확정하면 화면 입력뿐 아니라 로컬 저장 키를 삭제하고
저장을 꺼 버렸다.

**원인:** `PublicCalculatorContext.resetInputs()`가 입력 히스토리 초기화와 함께 `clearDraft()` 및 저장
상태 초기화를 직접 수행했다. 또한 빈 입력 저장값은 복원 시 의미 있는 값이 없다고 판단되어 저장 슬롯이
없는 것으로 표시될 수 있었다.

**✅ 수정 완료 (2026-07-28):**
- `resetInputs()`는 화면 입력만 기본값으로 바꾸며 로컬 저장 키·저장 설정을 건드리지 않는다.
- 저장이 켜져 있으면 기존 500ms 자동저장이 기본 입력값을 같은 공개 저장 키에 저장한다.
- 기본 입력값만 든 저장 초안도 존재하는 저장값으로 복원해 저장 아이콘·시각이 유지된다.
- 툴팁·확인창·저장 안내의 한영 문구를 실제 동작에 맞게 수정했다.
- 저장 중/저장 안 함 각각의 비파괴 동작과 빈 입력 복원 계약을 회귀 테스트로 고정했다.
