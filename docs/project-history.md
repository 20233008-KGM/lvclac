...
- 적용 커밋 `dev` **0272ceb**를 깨끗한 임시 작업 트리로 고정해 `awesome-s-projects1/lvclac-dev` Production **dpl_5dkF8CymY73mnyvT1oCJYMVK12Wy**로 배포했다. `devpilgrm.liqguard.com`은 새 배포 Ready·HTTP 200·noindex·DEV 배지·실제 클라우드 세트명 `삼성전자08`·콘솔 오류 0을 확인했고, `liqguard.com`은 별도 `lvclac` 프로젝트의 기존 **dpl_4TS5dVwcNtYpVtAfDSZ5oiRZQA8k** 그대로다. 현재 작업 폴더의 미커밋 export/i18n 변경은 배포에서 제외했다. Notion [Task LV-70](https://app.notion.com/p/3a526e6d586f81f4bf90cb4eedc3ce6f), [Work Log](https://app.notion.com/p/3a526e6d586f8128926fd1d0dd3ad0b3), Release Notes를 갱신했다. 푸시는 수행하지 않았고 결제 webhook·cron은 별도 smoke하지 않았다.
...
- `maintenance/public`의 최종 문구를 수동 이식해 유지·개시증거금률의 보유/신규 기준과 청산 위험, 약정가격의 평균 진입가격·거래 단위, 세팅 전후 공통 현재가 갱신 안내, 한국어 틱 사이즈의 현재가·주문가격, 영문 `Contract multiplier`와 나스닥→KOSPI200→원유 예시, 롱·숏 추가 주문의 양수·음수 방향을 `dev`에 반영했다. 공개판 전용 모달·가이드·수식·저장 동작은 가져오지 않았고 `rollPnlOnChange={setupComplete}` 계약은 유지했다.
...
- 적용 커밋 `dev` **6e04806**. 원본 추적 커밋은 `f063742`·`267614d`·`f9544b1`·`5642dcc`·`cd6db28`·`a05d9ea`·`161d9a0`·`d3a2758`·`a01f6b5`이며, 병행 중인 export 변경은 스테이징하지 않았다. Notion 완료 [Task](https://app.notion.com/p/3a426e6d586f815db326d6664c3764a8)와 [Work Log](https://app.notion.com/p/3a426e6d586f81908418e9cb0c294c99) 기록 완료. 푸시·배포는 수행하지 않았다.

**2026-07-21 — 기록장부·마이페이지 최근 기록에 슬롯명 표시**
- `dev` `/records`의 전체 슬롯 화면에서 중앙 시간 칩 아래에 현재 슬롯명을 표시하고, 슬롯 ID가 없으면 `미분류`, ID가 있지만 현재 목록에서 찾지 못하면 `슬롯 정보 없음 / Slot unavailable`으로 구분했다. 특정 슬롯·미분류 필터에서는 중복 슬롯명을 숨기며 기준일 라벨은 한영 모두 날짜만 남겼다. 행과 중앙 시간 묶음은 카드 세로 중앙을 정확히 공유하고, 모바일은 `시간 · 슬롯명` 한 줄로 전환한다.
- 마이페이지 계정 기록의 최근 스냅샷 제목 옆과 최근 주문 저장 시각 아래에도 같은 공용 판별 규칙을 적용했다. 주문 숫자 3열과 본문 폭은 유지하고 긴 이름에는 말줄임·전체 이름 툴팁을 제공한다. 검증: 집중 **45/45**·전체 Vitest **707/707**, TypeScript, 변경 파일 ESLint(기존 `MyPage.tsx` effect 규칙 제외), production build, diff check 통과. Chrome 한영 데스크톱·390/428px에서 전체/특정/미분류, 정상·긴 이름·미분류·조회 불가, 행 중심 일치와 가로 오버플로 0을 확인했다.
- 적용 커밋 `dev` **d188e0d**. Notion 완료 [Task LV-73](https://app.notion.com/p/3a426e6d586f81378c99cf15b6823623)와 [Work Log](https://app.notion.com/p/3a426e6d586f8110b756f35ff7db1467) 기록 완료. 푸시·배포는 수행하지 않았다.

**2026-07-22 — 마이페이지 최근 계좌스냅샷 5행 장부형 목록**
- `dev` `/my`의 계정 기록 요약이 조회 결과 첫 항목만 남기던 구조를 최신 5건 배열로 바꿨다. 중복 슬롯도 실제 시간순 그대로 유지하며 각 행에 저장 시각·슬롯명·계좌평가금·청산버퍼·레버리지를 표시하고, 헤더에는 단일 슬롯명 대신 표시 건수를 노출한다. 긴 슬롯명 툴팁과 미분류·조회 불가 공용 resolver를 재사용했고 결제 화면의 기존 3열 스냅샷 미리보기는 전용 modifier로 영향에서 분리했다.
- 검증: 집중 **16/16**·전체 Vitest **708/708**, TypeScript, production build, 변경 파일 ESLint(기존 `MyPage.tsx` effect 규칙 제외), diff check 통과. Chrome 실제 로그인 데이터와 fixture를 이용해 1920×855·428×844·390×844에서 5행, 두 요약 표 높이·정렬, 긴 슬롯명, 숫자 잘림, 페이지·표 가로 오버플로 0과 깨끗한 QA 탭 콘솔 오류 0을 확인했다.
- 적용 커밋 `dev` **c65f9eb**. Notion 완료 [Task LV-76](https://app.notion.com/p/3a526e6d586f81fe9fe8f8dd43ce9471)와 [Work Log](https://app.notion.com/p/3a526e6d586f81a0a10fe9fa2a10781e) 기록 완료. 별도 로컬 저장 안내 변경은 작업트리에 보존했고 푸시·배포는 수행하지 않았다.

**2026-07-22 — 숫자세트 자동 스냅샷을 ‘매일 기록’ UX로 명확화**
...
**2026-08-02 — 로그인·클라우드 초기 로딩 병렬화 및 운영 배포**
- `dev` **f0edebd**에서 `getSession`과 `INITIAL_SESSION` 이중 bootstrap을 단일 auth 이벤트 경로로 통합하고, 같은 계정의 반복 `SIGNED_IN`·토큰 갱신 수화를 차단했다. 로컬 세션 ID가 확인되면 계정 프로필·관리자·구독과 숫자세트·활성 세트를 병렬로 읽으며, 동일 이메일 프로필 update와 초기 클라우드 로딩 중 500ms 자동저장을 제거·차단했다. `/assets/*`에는 1년 immutable 캐시를 적용했다.
- 검증: 집중 **20/20**·전체 Vitest **821/821**, TypeScript 포함 production build, diff check 통과. 변경 파일 ESLint 3건은 수정 전 HEAD에도 같은 Context Fast Refresh·effect 규칙 오류임을 비교 확인했다. 운영 warm reload 3회는 인증 **573/599/624ms**, 클라우드 **573/714/625ms**였고 새 탭 2회는 인증·클라우드 **1,237/1,213ms**였다. startup Supabase GET 5건은 각 1회, 프로필 UPDATE 0건, 반복 새로고침 자산 12/12 cache hit·전송 0 bytes, 콘솔 경고·오류 0을 확인했다.
- `origin/dev` 푸시와 Vercel Production **dpl_DvBkv7HNNAMmETSj22K2pwak1rER** `READY`·`devpilgrm.liqguard.com` 반영 완료. 다른 국가·모바일 회선·저사양 기기 절대 시간은 미측정이며 현재 결과로 별도 startup RPC는 보류한다. 인증 변경은 public에 전파하지 않고 정적 자산 캐시만 별도 검토한다. Notion 완료 [Task LV-103](https://app.notion.com/p/3b026e6d586f812d84c0f748bd01962a)·[Work Log](https://app.notion.com/p/3b026e6d586f81168b09ce6d362b27f7)·QA / Test Plan·Release Notes 기록 완료.

**2026-08-12 — 미커밋 Paddle 공개 심사 페이지 운영 배포**
- `maintenance/public` 작업트리의 미커밋 요금제·약관·개인정보·환불·푸터·메타데이터·라우팅 변경을 Vercel `awesome-s-projects1/lvclac` Production에 배포했다. 첫 배포 **dpl_FLPpnPbGKY8qrUZyNsnNJ78duvJJ** 뒤 실제 화면에서 `/pricing`·`/refund-policy`가 기존 `vercel.json` 규칙 때문에 `/`로 307 이동하는 것을 발견해 두 리디렉트만 제거하고 공개 사이트맵과 middleware 테스트에 신규 URL을 추가했다. 최종 배포 **dpl_1Z8KsSTmB93kk7QBUqZyNpNZCbK8**가 `READY`이며 `liqguard.com` 별칭에 연결됐다.
- 검증: 전체 Vitest **764/764**, 변경 파일 ESLint, TypeScript 포함 production build, diff check 통과. 홈·요금제·약관·개인정보·환불·robots·sitemap·ads.txt HTTP 200, 1440×1000·390×844 운영 브라우저에서 요금제 3카드와 Paddle·Supabase 법률 문구·환불 절차, 가로 오버플로 0, 콘솔 경고/오류 0을 확인했다. 전체 ESLint는 기존 오류 **31건/경고 4건**으로 실패하며 Paddle Live 승인·실결제·해지·환불 E2E와 Git 커밋·푸시는 미수행이다. Notion [Task LV-7](https://app.notion.com/p/39926e6d586f8139a4f2dc9319179acb)·[Work Log](https://app.notion.com/p/3ba26e6d586f81258ba8c8b9b358fa9f)·[Release](https://app.notion.com/p/3ba26e6d586f81bfac26f1ebf9b7ee0f) 갱신 완료.
