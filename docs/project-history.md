...
**2026-08-11 — 메모 버튼 포커스 링 정렬**
- `dev` 1차 **0f06bff**의 `inset` 링만으로는 남아 있던 소수점 픽셀 좌표와 무효 테마 변수 참조를 **df7e727**에서 재수정했다. 숫자세트 팝업 좌표·내부 텍스트 행 높이를 정수 픽셀로 고정하고 메모 버튼에 실제 `--color-primary`·`--color-text-muted` 토큰을 사용했다. 버튼 크기·아이콘·클릭 영역·메모 열기 동작은 유지했다.
- 검증: 브라우저에서 삼성08 메모 버튼 **(416, 593) / 28×28**, `:focus-visible=true`, 1px inset 링 계산·표시 정상. 집중 **44/44**·전체 Vitest **840/840**, 변경 파일 ESLint, TypeScript 포함 production build, diff check 통과. 운영 배포와 배포 후 화면 확인은 미수행이며 public은 메모 기능이 없어 전파 대상이 아니다. 기존 LV-82 후속과 [Work Log](https://app.notion.com/p/3b826e6d586f8195bc8df3e573bc465c) 기록 정정 완료.
...
**2026-08-04 — Sandbox 본인 구독 즉시 해제 고정 도구**
- `dev` **53043f4**에서 Paddle Sandbox 구독 제어의 관리자 전용 조건을 제거하되 `PADDLE_ENV=sandbox` 서버 차단과 인증 사용자의 `subscriptions.user_id` 소유권 확인을 유지했다. 로그인한 활성·trialing 사용자는 언어·거래 종목 선택 아래 좌상단 `현재 로그인된 계정 구독 즉시 해제` 버튼을 보고, 확인 후 자기 테스트 구독만 즉시 종료하며 상태 새로고침 뒤 `/billing` Free 결제 화면으로 이동한다. `/billing` 관리자 도구에는 Paddle 상태 동기화만 남겨 중복 종료 UI를 제거했다.
- 검증: 집중 **32/32**·전체 Vitest **837/837**, TypeScript 포함 production build, 변경 파일 ESLint, diff check 통과. 로컬 브라우저 데스크톱·390×844에서 기존 좌상단 컨트롤 배치, 가로 오버플로 0, 콘솔 오류 0을 확인했다. 로그인된 active 구독 버튼의 실제 노출·클릭, push·Vercel 배포, Pro QA 즉시 종료→재결제 E2E는 미수행이다. 공개판은 결제·로그인이 없어 전파 대상이 아니다. Notion [Task LV-7](https://app.notion.com/p/39926e6d586f8139a4f2dc9319179acb)·[Work Log](https://app.notion.com/p/3b226e6d586f818b8c11f9fa95087465)·[Release](https://app.notion.com/p/3b226e6d586f812caba5f23d5e467598) 기록 완료.

**2026-08-12 — 공개 첫방문 필드 안내를 무이동 안내 모드로 전환**
- `maintenance/public` **e2cdc4e**에서 계산기 본문 위 `FieldHintBanner`와 닫기 버튼을 제거하고, 기존 상단 `사용법` 버튼을 `안내 중` 토글로 확장했다. 온보딩에서 고른 거래 상황에 맞춰 필요한 필드에 순서 번호를 배치해 계산기 높이에 영향을 주지 않으며, 첫 유효 청산가가 나오면 자동 종료되고 사용자가 다시 켤 수 있다. 후속 **36105dd**에서 번호의 옆 입력 열 침범과 주문 중복을 제거했고, 최종 **4bc6da7**에서 모든 번호를 라벨 내용이 끝나는 지점 바로 오른쪽에 배치했다. 주문 번호는 `주문` 제목에 한 번만 표시한다. DEV의 기존 배너는 수동 전파 Backlog로 분리했다.
- 검증: 전체 Vitest **783/783**, TypeScript 포함 production build, 변경 파일 ESLint, diff check 통과. 로컬 브라우저에서 첫방문 6단계·즉시 안내 활성·자동 종료·수동 재열기·콘솔 경고/오류 0을 확인했다. 데스크톱과 390×844 모바일 모두 안내 전환 전후 계산기 그리드 위치·높이와 상단 버튼 폭이 같고 모바일 가로 넘침은 0이었다. 최종 좁은 화면 좌표 검사에서도 모든 번호가 자기 필드 경계 안이면서 라벨 내용 직후, 주문 입력 라벨 중복 번호 0개, 가로 넘침 0이었다. push·Vercel 배포·운영 `liqguard.com` 검증은 미수행이다. Notion [Task](https://app.notion.com/p/3ba26e6d586f81e68b7ff330c1f7ee65)·[Work Log](https://app.notion.com/p/3ba26e6d586f81e9be4ed390221dc731)·[DEV Backlog](https://app.notion.com/p/3ba26e6d586f810885fbdccdbbe7dec1) 기록 완료.
**2026-08-14 — 공개·DEV 영어 페이지 초기 로딩 문구 현지화 및 배포**
- `maintenance/public` **843d9c8**은 경로별 정적 HTML의 `<html lang>`에 따라 영어 부트 문구 `Futures Calculator`와 한국어 `선물 계산기` 중 하나만 표시한다. `dev` **cfca9a2**는 `?lang`→`/en`→localStorage→sessionStorage→국가 쿠키→브라우저 언어의 기존 우선순위를 React보다 먼저 동기 적용해 저장 언어·브라우저 언어 기반 새로고침도 같은 결과를 낸다. DEV 로컬 증빙·임시 PDF를 Vercel 업로드에서 제외하는 **b50eaba**도 추가했다.
- 검증·배포: 공개 전체 Vitest **805/805**, DEV **893/893**, 양쪽 TypeScript 포함 production build와 Vercel 원격 build 통과. 공개 **dpl_Euz8oyUa8cVxRntULVz8Qbm78Ruv**는 `liqguard.com`, DEV **dpl_25JjF4bve5fFvSzTZFDLEKE7VQnq**는 `devpilgrm.liqguard.com` Production alias에 `READY`다. 두 실제 `/en` 도메인에서 외부 앱 JS를 차단한 초기 화면은 영어만 표시되고 `lang=en`·한국어 숨김·가로 넘침 0이었으며, 차단을 푼 깨끗한 새 탭은 부트 셸 제거·영어 제목·콘솔 오류 0을 확인했다. 두 브랜치 push는 미수행이다. Notion [QA](https://app.notion.com/p/3bc26e6d586f817a9c30ccfb4bd49e3a)·[Work Log](https://app.notion.com/p/3bc26e6d586f813a87aeeeab81c02546)·[Release](https://app.notion.com/p/3bc26e6d586f81f2b018e5b32f0915b5) 갱신 완료.

**2026-08-15 — 영어 고정 증거금 입력 패널 가로 오버플로 수정**
- `maintenance/public`의 영어 `Per` 모드에서 `Maintenance margin (per contract)` 라벨이 2열 `min-content` 최소폭을 강제해 패널과 input을 오른쪽 결과 패널 아래까지 확장하던 문제를 수정했다. 입력 2열을 `minmax(0, 1fr)`로 수축 가능하게 하고 증거금 섹션에 모드 클래스를 추가해 긴 라벨만 줄바꿈하며, 영어 Per 모드의 라벨 높이를 맞춰 두 input의 세로 시작선을 정렬했다. CSS·컴포넌트 계약 회귀 테스트도 추가했다.
- 전파·검증·배포: 공개 `maintenance/public` **9ebe70f·9987f6a**, DEV `dev` **b61fb8d·5cf6e3b**를 origin에 push했다. 최종 배포 소스에는 작업 중 함께 들어온 공개 **0762d91**, DEV **791acff·fd5771d**도 포함됐고, 그 뒤 생성된 로컬 공개 **c2d780c**·DEV **8a26203**은 각 branch가 origin보다 1 commit 앞서 있어 이번 배포에는 포함되지 않았다. 공개 작업 트리 Vitest **809/809**, DEV clean commit **902/902**, 양쪽 production build와 대상 회귀 테스트가 통과했다. 공개 **dpl_AThdbW5ZuJcad1xQNSQgvzuBgsaV**와 DEV **dpl_GKGfVL3Qevju7LLT7jMpncGWnQXw**는 각각 `liqguard.com`, `devpilgrm.liqguard.com` Production alias에서 `READY`다. 두 실제 `/en` 도메인의 1440px·390px 영어 Per 모드에서 document·panel·section 가로 오버플로 0, input 우측 경계가 field 안, 두 input 시작선 일치, 콘솔 경고·오류 0을 확인했다. 실제 iOS/Android 렌더링은 미검증이며 공개 clean commit 전체 테스트의 기존 `robots.txt` CRLF 단언 1건(796/797)과 npm audit high 3건은 별도 범위다. Notion [Task](https://app.notion.com/p/3bc26e6d586f81b3adb6fbe9bb919f1d)·[Work Log](https://app.notion.com/p/3bc26e6d586f8149bc88de077d381c2b)·QA / Test Plan·[Release](https://app.notion.com/p/3bc26e6d586f8119ab32e8d0674228e1) 갱신 완료.

**2026-08-27 — DEV 전체 기능판의 maintenance/public 통합 후보 구성**
- `maintenance/public`과 `dev`를 부모로 갖는 `codex/maintenance-dev-upgrade` 통합 브랜치를 만들고 DEV 최종 트리를 기준으로 로그인·클라우드 숫자세트·기록·마이페이지·피드백·관리자·결제 준비·cron API를 유지했다. 공개판의 정적 한영 SEO HTML, canonical·hreflang·sitemap, 업데이트 상세, 회사·문의·법무, Google Consent Mode, 광고 중지(`ADS_ENABLED=false`) 계약을 다시 적용했다. 기존 공개 저장 키를 기본 로컬 숫자세트로 1회 이관하는 회귀 계약과 비공개 경로 상시 noindex·sitemap 제외를 추가했다.
- 검증: 전체 Vitest **949/949**, TypeScript 포함 production build, 통합 변경 파일 ESLint, diff check 통과. 정적 공개 HTML **24/24**와 sitemap 비공개 경로 누출 0을 확인했다. 로컬 1440×1000·390×844 한영에서 7단계 안내, 가로 오버플로 0, 관리자 비로그인 차단, 광고 DOM 0, 동의 전 GA 0·분석 동의 후 GA 1, 콘솔 오류 0을 확인했다. 원격 DB는 읽기 전용으로만 비교했으며 10개 public 테이블 RLS와 사용자·숫자세트·구독·주문·스냅샷 기존 데이터를 확인했고 migration apply/repair는 하지 않았다.
- Vercel Preview **dpl_5fKF1yZYMahFC3pCC7wMDF7X7Wpt**는 `Ready`, 고정 별칭은 `lvclac-maintenance-dev-upgrade.vercel.app`이다. 공개·계정·기록·피드백·관리자 경로 200, `/kit` 307, `X-Robots-Tag: noindex, nofollow`, 정적 HTML의 광고·AdSense·GA 부트 스크립트 0을 확인했다. Paddle Sandbox에는 앱 가격 계약과 같은 `LiqGuard Pro` SaaS 제품, 월 **$5**, 연 **$48**, Preview client token을 만들었다.
- 완료 게이트는 통과하지 못했다. Paddle Sandbox API key가 Vercel·로컬 모두 비어 `/api/billing/*`가 `billing_not_configured`이고, Supabase 대시보드가 비로그인 상태라 Preview origin redirect allow-list를 적용하지 못했다. 따라서 성공·거절 결제, webhook 반영, 예약 해지와 인증 기반 클라우드 E2E는 미수행이며 계획의 실패 처리에 따라 로컬 `maintenance/public` fast-forward, origin push, Production 배포·환경·도메인 변경은 수행하지 않고 통합 브랜치만 보존했다. Notion [Plan](https://app.notion.com/p/3c926e6d586f81a38d3bcbb164ad1025)·[Task](https://app.notion.com/p/3c926e6d586f811a8a5fcd2ed745ce2c)·[Work Log](https://app.notion.com/p/3c926e6d586f8166a949fae402c8472b)와 QA / Test Plan을 갱신했다.

**2026-08-15 — 공개 첫방문 계산기 우선 노출·헤더 온보딩 CTA**
- `maintenance/public` **92828df**에서 신규 방문의 6단계 WelcomeFlow 자동 모달을 없애고 계산기 본체를 먼저 표시한다. 기존 `사용법` 위치에는 한영 `처음이신가요? · 1분 안내` CTA를 보여주며, 사용자가 누르면 기존 온보딩을 열고 완료하면 CTA를 기존 필드 안내 버튼으로 되돌린다. 온보딩 미완료 중에는 로컬 저장을 기본 OFF로 두고 개인정보 자동 모달도 보류해 계산은 즉시 가능하되 저장·분석·맞춤 광고는 기본 비활성 상태를 유지한다.
- 검증: 집중 **26/26**·전체 Vitest **812/812**, TypeScript 포함 production build, 변경 파일 ESLint, staged diff check 통과. 로컬 1280px·375×812 신규 origin에서 초기 dialog 0·계산기 노출·`저장 안 함` 기본·CTA 클릭 6단계 열림·완료 후 기존 안내 버튼 복귀·문서 가로 넘침 0·콘솔 경고/오류 0을 확인했다. push·Vercel Production 배포·실제 `liqguard.com`·실 iOS/Android는 미검증이다. DEV는 7단계·로그인·클라우드 계약이 다른 별도 제품이라 자동 전파하지 않았다. Notion [Task](https://app.notion.com/p/3bd26e6d586f81c1b1fff7f26e1578d8)·[Work Log](https://app.notion.com/p/3bd26e6d586f8186bfc2c527fd2d1779)·[QA](https://app.notion.com/p/3bd26e6d586f81309d9befb73fcce1fa) 기록 완료.

**2026-08-16 — 영문 수식 페이지의 약자 표기 제거**
- `maintenance/public` **79e2142**에서 `/en/formulas`의 `E₀`·`C₀`·`P`·`Q` 등 기호 중심 용어표와 모든 영문 수식을 `Account equity`·`Current price`·`Total sensitivity`처럼 풀어쓴 용어로 바꿨다. 한국어판과 같이 처음 온 사용자가 각 항을 용어표 없이도 읽을 수 있게 하되, 계산 로직과 수식의 뜻은 유지했다.
- 검증: 수식 카피 Vitest **3/3**, 변경 2파일 ESLint, TypeScript 포함 production build 통과. 로컬 `/en/formulas` 1280px와 390×844에서 수식 카드 가로 overflow 0·콘솔 오류 0을 확인했다. `liqguard.com` Production 배포와 실 iOS/Android는 미검증이다. DEV는 로그인·클라우드 기능이 있는 별도 제품이라 자동 전파하지 않았다. Notion [Work Log](https://app.notion.com/p/3be26e6d586f81adb898fb790b9048d3) 기록 완료.
...
**2026-08-27 — DEV 전체 기능판 → maintenance/public 통합 후보·Preview 차단**
- `maintenance/public`과 `dev`를 부모로 갖는 `codex/maintenance-dev-upgrade`에서 DEV 인증·클라우드·기록·관리자·피드백·결제 준비·cron 기능과 공개 정적 SEO·법무·Consent Mode·광고 중지 계약을 결합했다. 저장 이관·비공개 noindex 회귀를 추가했고 전체 Vitest **949/949**, production build, 통합 변경 파일 ESLint, diff check, 정적 HTML **24/24**, 로컬 1440px·390px QA를 통과했다.
- Vercel Preview **dpl_5fKF1yZYMahFC3pCC7wMDF7X7Wpt**는 Ready이며 `lvclac-maintenance-dev-upgrade.vercel.app` 별칭을 쓴다. Paddle Sandbox에 `LiqGuard Pro` SaaS 제품과 월 $5·연 $48 가격·Preview client token을 만들었다. 그러나 Paddle API key 부재로 billing API가 `billing_not_configured`이고 Supabase redirect allow-list도 비로그인으로 미적용이라 인증·클라우드·결제 E2E가 막혔다. 계획대로 로컬 `maintenance/public` FF, push, Production 변경은 하지 않고 통합 브랜치만 보존했다. Notion [Task LV-117](https://app.notion.com/p/3c926e6d586f811a8a5fcd2ed745ce2c)·[Work Log](https://app.notion.com/p/3c926e6d586f8166a949fae402c8472b)·QA 기록 완료.

**2026-08-29 — 공개·DEV 고정 언어/거래 종목 개발 컨트롤 제거**
- `maintenance/public` **fca44b6**과 `dev` **82fb6d3**에서 앱 bootstrap이 좌상단에 마운트하던 고정 한국어/English 토글과 거래 종목 프리셋 셀렉트를 모두 제거했다. 공개판의 언어 전환은 기존 계약대로 푸터 링크만 유지하며, DEV 숫자세트 내부의 저장 프리셋 기능과 UI kit 컴포넌트는 삭제하지 않았다.
- 검증·배포: 공개 집중 **2/2**·전체 Vitest **953/953**, DEV 집중 **1/1**·전체 **882/882**, 양쪽 TypeScript 포함 production build·변경 파일 ESLint·diff check 통과. 공개 `2f1c122`는 Production **dpl_435gU9ALbZX5vfLK2FdBnQaFmThH**로 `liqguard.com`에, DEV `82fb6d3`은 **dpl_hDEe17CCnJWaPLJfL9uHSJoucMyd**로 `dev.liqguard.com`·`devpilgrm.liqguard.com`에 배포했다. 실제 데스크톱·390×844에서 고정 언어/프리셋 컨트롤 0개, 푸터 English 링크 1개, 가로 넘침·콘솔 경고/오류 0을 확인했고 DEV 배지와 `noindex, nofollow`도 유지됐다. 배포용 임시 worktree는 정리했다. 당시 제외했던 공개 후속 `8a68355`·`625e233`은 테스트 계정 초기화 제거 `60edc3e`와 함께 최종 Production **dpl_DBRNMf3aMWx5m3t8mTdrWEy4kjJm**에 포함했다.
