...
- 데스크톱을 **왼쪽 제목/한 줄 설명 + 오른쪽 1060px 장부** 2열로 재배치하고, 우측 여백을 1vw(최대 24px)로 줄여 장부를 오른쪽으로 밀면서도 설명과 겹치지 않게 했다. 툴바의 스냅샷·주문 표시 개수 문구는 제거하고 조작 버튼만 남겼으며, 기록 영역 높이는 `100dvh - 280px`로 늘려 화면 하단의 큰 빈 공간을 없앴다. 모바일은 높이를 자동으로 되돌려 기존 한쪽 타임라인·페이지 스크롤을 유지.
...
- 검증: tsc 통과, vitest **617/617**, build 통과. 실제 앱 CSS를 쓰는 임시 렌더 하네스로 1867×900 데스크톱에서 설명-장부 간격과 하단 여백 제거를 확인하고 하네스 삭제. 대상 eslint는 기존 `RecordsArchivePage` 기준선 오류 3건만 잔존.
