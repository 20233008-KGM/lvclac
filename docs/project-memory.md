...
**스택/인프라**: React + TypeScript + Vite 프런트, Supabase(DB·Auth), Vercel(호스팅·`/api/*` 서버리스 함수·크론). 인증메일 발신 auth@liqguard.com(Resend, 한/영 분기). 테스트 vitest 616개. **주의: Vercel은 api/ TS를 파일단위 컴파일만 함 → 서버 상대 import는 반드시 `.js` 확장자**(serverEsmImports.test.ts가 자동 검사).
...
**2026-07-16 — 기록 장부 시안 레이아웃을 실제 /records에 적용 + 6열 정렬 복원**
...
- 데스크톱을 **왼쪽 제목/한 줄 설명 + 오른쪽 1120px 장부** 2열로 재배치하고, 장부 툴바·열 헤더는 고정한 채 기록 목록만 최대 520px 내부 스크롤. 전체 페이지의 1600px 중앙 폭 제한을 해제하고 우측 여백을 4.5vw(최대 92px)로 고정해 넓은 화면에서도 장부가 오른쪽 끝선에 맞는다. 모바일은 기존 한쪽 타임라인·페이지 스크롤 유지.
...
- 검증: tsc 통과, vitest **616/616**, build 통과. 실제 앱 CSS를 쓰는 임시 렌더 하네스로 1600×900 데스크톱·428×900 모바일 확인 후 하네스 삭제. 대상 eslint는 기존 `RecordsArchivePage` 기준선 오류 3건만 잔존.
