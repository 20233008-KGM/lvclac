...
- **세션 종료 시 아래 `## 최근 진행상황` 섹션을 이번 세션에서 한 일 기준으로 갱신합니다**(무엇을 했고, 다음에 이어할 게 뭔지 간단히). 새 세션은 시작할 때 이 섹션부터 읽어 현재 상태를 파악하고, 매번 사용자에게 진행상황을 다시 물어보지 않습니다.
...
_세션 종료 시 이 섹션을 최신 내용으로 덮어써 주세요. 날짜 + 한 일 + 다음 할 일 위주로 짧게._
