...
describe('calculator focus-complete history wiring', () => {
  ...
  it('keeps public autosave driven by global inputs instead of DOM blur', () => {
    const context = source('src/context/PublicCalculatorContext.tsx')

    expect(context).toContain('const savedAt = persistDraft(inputs)')
    expect(context).toContain('}, [inputs, saveEnabled])')
  })
