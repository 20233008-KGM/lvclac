...
export const ko: Messages = {
  ...
  siteTitleTooltip: {
    ...
    usageTitle: '활용 방법',
    usageBody:
      ' HTS·MTS의 계좌 평가금·증거금률로 청산가를 확인하거나, 주문 계약수·가격으로 체결 후 상태를 미리 볼 수 있습니다.\n매일은 시나리오 가격에 시세만 넣고 Enter → Enter로 평가금액을 갱신할 수 있습니다.',
  ...
  howToUse: {
    ...
    beginnerBody:
      ' 계좌 평가금액과 유지·개시 증거금률을 입력한 뒤, 아래 「주문」에서 계약수·가격을 조정하면 체결 직후 상태를 미리 볼 수 있습니다.\n시세만 바뀐 날에는 시나리오 가격으로 평가금액을 갱신할 수 있습니다.\n최초 입력은 변동성이 낮을 때 하고, 계좌평가금액·상품현재가를 한 화면에서 스냅샷하는 것을 권장합니다.',
    experiencedTitle: '이미 선물거래를 하시나요?',
    experiencedBody:
      ' 계좌·종목·증거금은 한 번만 맞춰 두세요.\n매일 아침 시세를 시나리오 가격에 넣고 Enter → Enter 하면 평가금액까지 손익이 반영됩니다.\n「이 기기에 입력값 저장」을 켜 두면 편합니다.\n최초 세팅은 변동성이 낮을 때, 계좌평가금액·상품현재가를 한 화면에서 스냅샷하는 것을 권장합니다.',
  ...
  orderScenarioHint:
    '계약수·주문 가격을 입력한 뒤 ↵으로 시뮬레이션합니다.\n\n· ↵ — 주문 반영 미리보기\n· ✓ — 계좌·보유계약수에 확정 (재주문 아님)\n· Esc — 취소\n· Ctrl+Z — 확정 취소 후 미리보기 복귀',
  clearAllInputs: '비우기',
  clearAllInputsHint: '포커스된 입력란은 Delete 키로 값을 초기화할 수 있습니다.',
  ...
  draftSave: {
    label: '이 기기에 입력값 저장',
    hint: '켜면 입력값이 이 브라우저에만 저장되고, 다음 방문 시 불러옵니다. 서버로 전송되지 않습니다. 끄면 저장된 값이 삭제됩니다.\n\n어제 맞춰 둔 계약수·증거금을 유지한 채, 다음 날 시나리오 가격으로 시세만 반영하기에 좋습니다.',
  ...
  marginMode: {
    ...
    perContract: '계약당',
    total: '총액',
    rateHint: '약정가치 대비 비율로 입력 (국내 선물)',
    perContractHint: '1계약당 고정 금액.\n총액 = 계약당 × 계약수 (해외 선물)',
    totalHint: 'HTS·MTS에 표시된 총 증거금을 그대로 입력',
  },
  fields: {
    accountEquity: {
      label: '계좌 평가금액',
      hint: '예탁금에 미결제 손익을 더한 금액입니다.\nHTS·MTS의 「계좌 평가금액」과 같습니다.\n\n일상 갱신: 포지션·증거금이 그대로면 시나리오 가격으로 시세만 반영하면 됩니다.\n\n최초 입력: 평가금과 현재가는 한 세트로 맞춰야 합니다.\n시세가 요동치면 입력 사이에 소폭 어긋날 수 있으니, 변동성이 낮을 때 입력하거나 계좌평가금액, 상품현재가가 한 화면에서 스냅샷하시는 걸 권장합니다.',
    ...
    entrustedMargin: {
      label: '위탁증거금 (총액)',
      hint: 'HTS에 표시된 총 개시·위탁증거금.\n계약수에 비례 조정',
      placeholder: '12,000',
    },
    entrustedMarginPerContract: {
      label: '개시증거금 (계약당)',
      hint: '1계약당 고정 개시·위탁증거금 (해외 선물)',
      placeholder: '6,000',
    },
    contracts: {
      label: '보유 계약수',
    ...
    currentPrice: {
      label: '현재가',
      hint: '기준 시세입니다.\n\n※ 여기만 바꾸면 평가금액은 변하지 않습니다. 손익 반영은 시나리오 가격을 사용하세요.\n\n최초 입력: 평가금과 같은 순간의 시세를 넣으세요. HTS는 한 화면에서, MTS는 잔고·시세 화면을 빠르게 오가거나 스냅샷 기준으로 넣으세요. 변동성이 클 때는 시세가 잔잔할 때 입력하는 것을 권장합니다.',
      placeholder: '35,000',
    },
    scenarioPrice: {
      label: '시나리오 가격',
      hint: '가격이 바뀌었을 때 계좌 상태를 미리 보거나, 매일 시세만 반영해 평가금액을 갱신합니다.\n\n단축키\n· Enter(↵) — 시나리오 모드 진입 (미리보기)\n· Enter(↵) — 손익 반영 (현재가·평가금액 갱신)\n· Esc — 종료 및 복원\n· Ctrl+Z — 손익 반영 취소\n\n💡 틱 사이즈 입력 시: 빈 칸에는 현재가가 표시되고, ▲▼ 스테퍼를 누르면 현재가 기준으로 가격이 바로 입력됩니다.\n\n💡 매일 아침: 시세 입력 → Enter → Enter (계약수·증거금은 그대로)\n\n최초 세팅: 평가금과 현재가는 한 세트로 맞춰야 합니다.\n시세가 요동치면 입력 사이에 소폭 어긋날 수 있으니, 변동성이 낮을 때 입력하거나 계좌평가금액, 상품현재가를 한 화면에서 스냅샷하시는 걸 권장합니다.',
      placeholder: '변동 가격',
    },
    tickSize: {
      label: '틱 사이즈',
  ...
  legal: {
    ...
    termsArticles: [
      ...
      {
        title: '제4조 (입력값의 저장)',
        body: '계산기 입력값과 일부 화면 설정(패널 너비 등)은 저장 기능을 켠 경우에만 해당 기기의 브라우저 저장소(localStorage)에 보관됩니다. 해당 정보는 서버로 전송되지 않으며, 저장 기능을 끄면 기기에 보관된 값이 삭제됩니다.',
      },
      {
        title: '제5조 (광고)',
    ...
    privacyArticles: [
      {
        title: '1. 수집하는 정보',
        body: '계산기 입력값 및 화면 설정(패널 너비 등)은 저장 기능을 켠 경우에만 이용자 기기의 브라우저 저장소에 보관됩니다. Google Analytics·AdSense 등을 설정한 경우 쿠키 및 접속 로그가 자동으로 수집될 수 있습니다.',
      },
      {
        title: '2. 이용 목적',
      ...
      {
        title: '3. 보관 및 처리',
        body: '입력값은 저장 기능을 켠 경우에만 이용자 기기에 보관되며, 회사 서버로 전송되지 않습니다. 저장 기능을 끄면 해당 기기에 보관된 값이 삭제됩니다.',
      },
      {
        title: '4. 제3자 제공',
      ...
      {
        title: '5. 이용자 권리',
        body: '저장 기능을 끄거나 브라우저 데이터를 삭제하여 기기에 보관된 입력값을 제거할 수 있습니다.',
      },
      {
        title: '6. 문의',
